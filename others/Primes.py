"""
    ...
    If y == int(math.sqrt(N)) is not the largest , then there is x > y ==> x*y > N.So y is the largest prime .
    ...
    def get_primes(self, n):
        ...
        for i in range(2, n):
            if not arr[i]:
                continue
            primes.append(i)
            # here j can start as i*i, better than 2*i. Let's say i = 5, we can direct consider 25 .
